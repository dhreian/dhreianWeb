...
function getSigningSecret() {
  const secret =
    process.env.PLUGIN_DOWNLOAD_SECRET ||
    process.env.RATE_LIMIT_SECRET ||
    process.env.RESEND_API_KEY ||
    'dhreian-default-download-secret-fallback-2026';
...
export function createPluginDownloadToken(pluginKey, version) {
  const payload = {
    pluginKey,
    version,
...
export function verifyPluginDownloadToken(token) {
  try {
    ...
    const hasInvalidVersion =
      ...
      (typeof payload.version !== 'string' || !/^[0-9]+(?:\.[0-9]+){0,3}$/.test(payload.version));

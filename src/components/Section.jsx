...
export function Section({
  ...
  return (
    <section id={id} className={`py-12 md:py-16 px-4 md:px-8 relative overflow-hidden ${className}`}>
      {glow && (
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[85%] max-w-4xl h-[300px] rounded-full blur-[140px] pointer-events-none opacity-40 transition-all duration-1000 transform-gpu bg-purple-900/20"
        />
      )}
      ...
      {divider && (
        <div
          className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2/3 md:w-1/2 h-px bg-gradient-to-r from-transparent via-purple-500/25 to-transparent pointer-events-none"
        />
      )}
    </section>
  );
...
export function SectionHeading({
  ...
  const neonClass = 'neon-text-purple';
    ...
    <div className={`mb-12 md:mb-16 ${alignClass}`}>
      {eyebrow && (
        <div className={`flex items-center gap-3 mb-5 ${eyebrowJustify}`}>
          <span className="h-px w-10 bg-gradient-to-r from-purple-500 to-transparent"></span>
          <span className={`font-body ${accentColor} tracking-[0.3em] text-xs uppercase font-semibold`}>
            {eyebrow}
          </span>
          <span className="h-px w-10 bg-gradient-to-l from-purple-500 to-transparent"></span>
        </div>
      )}
